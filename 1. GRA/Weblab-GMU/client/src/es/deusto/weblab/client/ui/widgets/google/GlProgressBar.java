...
public class GlProgressBar extends Widget implements GlResizableWidget {
  ...
  public abstract static class TextFormatter {
     ...
     */
    protected abstract String getText(GlProgressBar bar, double curProgress);
  }
  
  
  /**
   * In segment mode, a moving fixed-size rectangle will be displayed
   */
  private boolean fixedBarLengthMode = false;
  
  /**
   * Length of the segment in fixedBarLengthMode. 1 is the full bar, 0.2 is 20% of the bar.
   */
  private double segmentWidth = 0.2;
   ...
   * @param curProgress the current progress
   */
  public GlProgressBar(double curProgress) {
   ...
   */
  public GlProgressBar(double minProgress, double maxProgress) {
   ...
   * @param curProgress the current progress
   */
  public GlProgressBar(double minProgress, double maxProgress, double curProgress) {
  ...
  public boolean getFixedLengthBarMode() {
	  return this.fixedBarLengthMode;
  }
  
  public void setFixedLengthBarMode(boolean enable) {
	  this.fixedBarLengthMode = enable;
  }
...
public void setProgress(double curProgress) {
    ...
    if(!this.fixedBarLengthMode) {
    	DOM.setStyleAttribute(this.barElement, "width", percent + "%");
    } else {
    	int leftOffset = percent;
    	DOM.setStyleAttribute(this.barElement, "left", leftOffset + "%");
    	
    	// Calculating the width of the bar is harder, because we need to make
    	// sure that at most left + width = 100.
    	final int segmWidth = (int) (100 * this.segmentWidth);
    	final int width = Math.min(100 - leftOffset, segmWidth);
    	DOM.setStyleAttribute(this.barElement, "width", width + "%");
    }
  ...
  protected void onLoad() {
    ...
    DOM.setStyleAttribute(getElement(), "position", "relative");
    GlResizableWidgetCollection.get().add(this);
    redraw();
  }

  @Override
  protected void onUnload() {
    GlResizableWidgetCollection.get().remove(this);
  }
